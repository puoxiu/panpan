...
var (
	...
	upload_FileIdPrefix       = "upload::file:id:"
	upload_FileFileSha1Prefix = "upload::file:fileSha1:"
)
...
func (m *defaultFileModel) Delete(ctx context.Context, id int64) error {
	...
	fileFileSha1Key := fmt.Sprintf("%s%v", upload_FileFileSha1Prefix, data.FileSha1)
	fileIdKey := fmt.Sprintf("%s%v", upload_FileIdPrefix, id)
...
func (m *defaultFileModel) FindOneByFileSha1(ctx context.Context, fileSha1 string) (*File, error) {
	fileFileSha1Key := fmt.Sprintf("%s%v", upload_FileFileSha1Prefix, fileSha1)
	var resp File
	err := m.QueryRowIndexCtx(ctx, &resp, fileFileSha1Key, m.formatPrimary, func(ctx context.Context, conn sqlx.SqlConn, v any) (i any, e error) {
...
func (m *defaultFileModel) Insert(ctx context.Context, data *File) (sql.Result, error) {
	fileFileSha1Key := fmt.Sprintf("%s%v", upload_FileFileSha1Prefix, data.FileSha1)
	fileIdKey := fmt.Sprintf("%s%v", upload_FileIdPrefix, data.Id)
...
func (m *defaultFileModel) formatPrimary(primary any) string {
	return fmt.Sprintf("%s%v", upload_FileIdPrefix, primary)
}
